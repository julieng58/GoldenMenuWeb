    ...
    $(document).ready(function() {
        ...
        const prixCarte = document.getElementById('textNewPrixCarte');

        prixCarte.addEventListener('change', updateValue);

        function updateValue(e) {
            const suggest = document.getElementById('suggestNewPrixPD');
            const suggestEPD = document.getElementById('suggestNewPrixEPD');
            var prix = (parseFloat(prixCarte.value)+8)*0.85;
            var prixEPD = (parseFloat(prixCarte.value)+16)*0.85;
            console.log(prix);
            suggest.value = Math.round(prix);
            suggestEPD.value = Math.round(prixEPD);
        }
        ...
        submitBtn.click(function () {
            ...
            var docDataNew = {
                ...
                prixPD: parseFloat($('#textNewPrixPD').val()),
                prixEPD: parseFloat($('#textNewPrixEPD').val()),
                dispo: booleanDispo,
            ...
            var newEntree = database.collection("PlatCOVID").doc();
        ...
        function clearInputs() {
            ...
            $('#textNewPrixPD').val('');
            $('#textNewPrixEPD').val('');
            $('#suggestNewPrixPD').val('');
            $('#suggestNewPrixEPD').val('');
        ...
        sgtTableElement.on('click', '.edit-btn', function () {
            ...
            var platFirestoreRef = database.collection("PlatCOVID").doc(plat_ID);
            platFirestoreRef.get().then(function(doc) {
                if (doc.exists) {
                    ...
                    $('#textPrixPD').val(doc.data().prixPD);
                    $('#textPrixEPD').val(doc.data().prixEPD);
        ...
        function platEdit(platID) {
            database.collection("PlatCOVID").doc(platID).set({
                nom: $('#textNomPlat').val(),
                prixCarte: parseFloat($('#textPrixCarte').val()),
                prixPD: parseFloat($('#textPrixPD').val()),
                prixEPD: parseFloat($('#textPrixEPD').val()),
        ...
        sgtTableElement.on('click', '.delete-btn', function () {
            var platFirestoreRef = database.collection("PlatCOVID").doc($(this).data('id'));
    ...
    function onLoadFunctions() {
        ...
        table.innerHTML = "";
        database.collection("PlatCOVID").get().then((querySnapshot) => {
            const table = document.getElementById("contenuPlats");
            querySnapshot.forEach((doc) => {
                ...
                let prixPD = row.insertCell(2);
                prixPD.innerHTML = doc.data().prixPD;
                let prixEPD = row.insertCell(3);
                prixEPD.innerHTML = doc.data().prixEPD;
                let dispo = row.insertCell(4);
                dispo.innerHTML = doc.data().dispo;
                let vegan = row.insertCell(5);
                vegan.innerHTML = doc.data().vegan;
                let glutenfree = row.insertCell(6);
                glutenfree.innerHTML = doc.data().glutenFree;
                let boutons = row.insertCell(7);
